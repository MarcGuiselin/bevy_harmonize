...
pub async fn build(
    ...
    // All manifests export wasm should be generated by now
    for source in sources.iter_mut() {
    ...
    // TODO: run `wasm-bindgen target/wasm32-unknown-unknown/release/my_cube_test.wasm --out-dir ./test`
    ...
    for source in sources.iter() {
        let wasm_path =
            dir.wasm_dest
                .join(format!("{}{}.wasm", source.name, ModSource::EXPORT_SYSTEMS));
        let dest_path = dir.dest.join(format!("{}", source.name));
        fs_utils::rename(&wasm_path, &dest_path).await?;
        wasm_files.push(dest_path);
    }
...
impl ModSource {
    ...
    async fn codegen_final(&mut self, dir: &Directories) -> Result<()> {
        ...
        let components: Vec<_> = TypeAddress::from_type_signatures(manifest.types.iter())
            .enumerate()
            .map(|(id, typ_address)| templates::ImportsComponent {
                name: typ_address.ty.stable_id().name,
                id: id as u32,
                address: typ_address.address,
            })
            .collect();
    ...
    async fn write_manifests(&mut self, dir: &Directories) -> Result<()> {
        let path = dir
            .wasm_dest
            .join(format!("{}{}.wasm", self.name, Self::EXPORT_MANIFEST));
