...
pub fn derive_addressable(input: TokenStream) -> TokenStream {
    ...
    let gen = quote! {
        ...
        unsafe impl Addressable for #ident {
            // Each bevy_harmonize mod gets its own __codegen crate

            const COMPONENT_ID: usize = __codegen::__resolve_component_id::<#ident>();

            // 1. Before resolving the manifest, __codegen::__resolve_address returns a dangling placeholder pointer
            ...
            const PTR: *mut Self = __codegen::__resolve_address::<#ident>();
        }
    };
